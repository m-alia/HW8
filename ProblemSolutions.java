 ...
 ********************************************************************/
 ...
 class ProblemSolutions {
     ...
     public boolean canFinish(int numExams, 
        ...
        boolean[] addedToStack = new boolean[numExams];                        
        // checking for cycles, depth first traversal
        // one array stores visited nodes
        // second array to store nodes visited while going through adjacency list

        for (int i = 0; i < numExams; i++) {
            if (!visited[i]) { // for each unvisited node
                Stack<Integer> stack = new Stack<>();
                stack.push(i);
                // push next node onto the stack
                while (!stack.isEmpty()) {
                    int v = stack.peek();
                    if (!visited[v]) {
                        visited[v] = true;
                        addedToStack[v] = true;
                        // if node hasn't been traversed yet, mark as traversed and still on stack
                        Iterator<Integer> j = adj[v].listIterator(); // iterating over adjacency list
                        while (j.hasNext()) {
                            int n = j.next();
                            if (!visited[n]) {
                                stack.push(n);
                            } else if (addedToStack[n]) {
                                return false;
                                // basically, check through the adjacency list for every unvisited node
                                // if at any point there's a node in adjacency list
                                // that points to an already marked node, this means there's a cycle
                                // so return false
                            }
                        } 
                        // for the entire adjacency list of node being checked, check if a node is already on the stack
                        // if at any point node in adjacency list points to an already visited node this means
                        // there is a cycle
                    } else {
                        stack.pop();
                        addedToStack[v] = false;
                    } // if no cycles are detected, pop adjacency list nodes off of stack, and unmark
                    // as being on the stack
                }
                
            } // check the adjacency list for any unvisited nodes
        }
        // for each node, perform a DFS

         // ADD YOUR CODE HERE - ADD YOUR NAME / SECTION AT TOP OF FILE
         return true;
        // if cycle isn't found, then it is possible to take all exams
            // so return true
     }
     ...
     public int numGroups(int[][] adjMatrix) {
         ...
         // example of creating adjacency list representation of adjacency matrix
         ...
         } // use depth traversal to traverse entire graph
         // calling external helper method to make counting easier
            // method initially calls helper, which traverses as many connected
            // nodes as possible, until it needs to backtrack
            // when no more connected nodes can be found, incremements the number of groups

         // YOUR CODE GOES HERE - you can add helper methods, you do not need
         // to put all code in this method.
         return numOfGroups; // return the final total number of groups
     }

     private void DFSUtil(int v, Map<Integer, List<Integer>> graph, boolean[] visited) {
        visited[v] = true;
        // when called, mark node as visited

        for (int n = 0; n < graph.size(); n++) {
            if (!visited[n]) {
                DFSUtil(n, graph, visited);
            }
        } // depth first traversal of graph with recursion
        // traverses every next unexplored node, if can't find unexplored node
        // then backtrack, and search for unexplored nodes
     } 
